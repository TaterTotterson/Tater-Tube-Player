...
if [ -f "${tater_repo_dir}/dist/steam/linux-x86_64/compliance/bundled-ffmpeg.txt" ]; then
    cp "${tater_repo_dir}/dist/steam/linux-x86_64/compliance/bundled-ffmpeg.txt" \
        "${tater_bundle_dir}/"
fi
...
SHA256SUMS. The build Containerfile, notices, FFmpeg binary evidence, and Qt
relinking instructions are included alongside them. No local patches were
applied to Qt or FFmpeg.

The official Qt 6.11.2 SPDX record labels its FFmpeg package 7.1.3, but the
shipped libavformat ABI is 61.7.103 and its embedded configuration prefix names
FFmpeg 7.1.5. Those binary values match the FFmpeg 7.1.5 source retained here.
EOF
