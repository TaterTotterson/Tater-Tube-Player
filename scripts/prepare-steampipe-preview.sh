...
tater_depot_dir="$tater_repo_dir/dist/steam/linux-x86_64"
tater_output_dir="$tater_repo_dir/dist/steam/steampipe"
...
if [ ! -x "$tater_depot_dir/tater-tube-player" ]; then
    echo "Build and audit dist/steam/linux-x86_64 before preparing SteamPipe." >&2
    exit 1
fi
...
case "$(uname -s)" in
    ...
    *)
        if ! command -v docker >/dev/null 2>&1 || \
            ! docker image inspect tater-tube-player-steam-builder:latest >/dev/null 2>&1; then
            echo "The non-Linux host needs the tater-tube-player-steam-builder image to audit the depot." >&2
            echo "Build the image as documented in packaging/steam/README.md, then retry." >&2
            exit 1
        fi
        docker run --rm --platform linux/amd64 \
            --volume "$tater_repo_dir:/workspace" \
            tater-tube-player-steam-builder:latest \
            ./scripts/audit-steam-depot.sh /workspace/dist/steam/linux-x86_64
        ;;
esac
