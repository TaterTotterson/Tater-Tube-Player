# Tater Tube Player — Steam store page draft

This is the paste-ready English store copy for the first Steam release. Text in
the actual Steam description intentionally contains no external links; use the
dedicated Steamworks link fields for the website and support URLs.
...
- Source code: `https://github.com/TaterTotterson/Tater-Tube-Player`
- Price: `Free`
- Primary language: English
- Initial platform: SteamOS / 64-bit Linux
- Tater Tube Server requirement: version 1.4.33 or newer

## Short description

Bring your self-hosted Tater Tube Server to Steam and Steam Deck with an artwork-first, controller-friendly player for your library, live channels, resume playback, and Tater Picks.
...
## Feature list for Steamworks

- Controller and Steam Deck navigation
- Self-hosted Tater Tube Server pairing
- Movies, television series, seasons, and episodes
- Continue Watching and server-backed viewing progress
- Artwork caching and fast cached library shelves
...
Use only tags Steamworks actually offers for software. Start with the nearest
available equivalents to:
...
- Software
- Controller
- Remote Play / Living Room (only if Steamworks offers an accurate tag)

Do not select `Steam Deck Verified`; Valve assigns Deck compatibility after its
own review. We can state that the interface is designed for Steam Deck.

## System requirements draft

### Minimum

- OS: SteamOS 3 or another supported 64-bit Linux distribution
...
- Additional notes: Requires an accessible Tater Tube Server 1.4.33 or newer. Hardware video decoding is recommended.

### Recommended

- OS: Current SteamOS on Steam Deck or a current 64-bit Linux distribution
- Processor: 64-bit quad-core processor
- Memory: 8 GB RAM
- Graphics: Hardware-accelerated H.264 and HEVC decoding
- Network: Wired Ethernet or 5 GHz Wi-Fi
- Storage: 1 GB available space
- Additional notes: Server transcoding requirements depend on the media formats and playback device.

Validate these values against the final self-contained release package and at
least one non-Deck Linux machine before entering them in Steamworks.

## Disclosures that should be visible to customers

- Tater Tube Server is required, separately installed, and not included.
